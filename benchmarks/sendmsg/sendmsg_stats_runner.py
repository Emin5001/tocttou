...
binaries = ["bin/sendmsg_copy", "bin/sendmsg_no_copy", "bin/sendmsg_cow"]
...
counter_map = {
    "bin/sendmsg_copy": np.zeros((len(sizes), len(counters) + 1), dtype=np.int64),
    "bin/sendmsg_no_copy": np.zeros((len(sizes), len(counters) + 1), dtype=np.int64),
    "bin/sendmsg_cow": np.zeros((len(sizes), len(counters) + 1), dtype=np.int64)
}
...
df_copy = pd.DataFrame(counter_map["bin/sendmsg_copy"], columns=columns)
df_no_copy = pd.DataFrame(counter_map["bin/sendmsg_no_copy"], columns=columns)
df_cow = pd.DataFrame(counter_map["bin/sendmsg_cow"], columns=columns)

df_copy.to_csv("sendmsg_copy_results.csv", index=False)
df_no_copy.to_csv("sendmsg_no_copy_results.csv", index=False)
df_cow.to_csv("sendmsg_cow_results.csv", index=False)
